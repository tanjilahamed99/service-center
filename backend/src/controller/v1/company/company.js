...
exports.getAllSparePartTransactions = async (req, res) => {
  try {
    const company = req.user._id;
    const { search } = req.query;
    ...
    let sparePartFilter = {};
    if (search) {
      const matchingParts = await SparePart.find({
        company,
        spareName: { $regex: search, $options: "i" },
      }).select("_id");
      sparePartFilter = { sparePart: { $in: matchingParts.map((p) => p._id) } };
    }
    ...
    const transactions = await SparePartTransaction.find({
      ...
      .populate("sparePart", "spareName brand product unit")
      .populate("fromId", "name")
      .populate("toId", "name")
      .populate("job", "complaintNumber")
      .sort({ createdAt: -1 })
    ...
    console.error("getAllSparePartTransactions error:", error);
    return res.status(500).json({
      success: false,
      message: "Failed to fetch transactions",
      error: error.message,
...
exports.getSparePartStockByCenter = async (req, res) => {
  try {
    const company = req.user._id;
    const { search } = req.query;
    ...
    const stock = await SparePartStock.find({
      ...
      .populate("sparePart", "spareName brand product modelNumber unit")
      .populate("ownerId", "name")
      .sort({ "ownerId.name": 1 });
