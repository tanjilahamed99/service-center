...
import { getServiceEngineers, deleteServiceEngineer } from "@/actions/company";
...
export default function ServiceEngineersPage() {
  ...
  async function handleDelete(engineer) {
    ...
    try {
      ...
      alert(err.response?.data?.message || "Failed to delete service engineer.");
    } finally {
      setDeletingId(null);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-navy-900">Service Engineers</h2>
          <p className="mt-1 text-sm text-slate-500">Manage the engineers assigned to your service centers.</p>
        </div>
        <Link
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-6 text-center text-slate-400">No service engineers yet.</td></tr>
            ) : (
              engineers.map((e) => (
                <tr key={e._id} className="border-t border-slate-100">
                  <td className="px-4 py-3 font-medium text-navy-900">{e.name}</td>
                  <td className="px-4 py-3 text-slate-600">{e.serviceCenter?.name || "—"}</td>
                  <td className="px-4 py-3 text-slate-600">{e.contactNumber || "—"}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      <button
                        type="button"
                        onClick={() => router.push(`/company/service-engineers/${e._id}/edit`)}
