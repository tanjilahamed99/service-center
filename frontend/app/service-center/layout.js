...
import { LayoutGrid, ListChecks, Users, UserCircle, LogOut, Menu, Bell } from "lucide-react";

const NAV_ITEMS = [
  { label: "Dashboard", href: "/service-center", icon: LayoutGrid },
  { label: "Jobs", href: "/service-center/jobs", icon: ListChecks },
  { label: "Service Engineers", href: "/service-center/engineers", icon: Users },
  { label: "Profile", href: "/service-center/profile", icon: UserCircle },
];

function isActive(href, pathname) {
  return href === "/service-center" ? pathname === "/service-center" : pathname.startsWith(href);
}
    ...
    <div className="flex h-full flex-col bg-navy-900">
      ...
      <div className="flex items-center gap-3 px-5 py-6">
        <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-electric-500/15 ring-1 ring-electric-400/40">
          <svg viewBox="0 0 24 24" className="h-4.5 w-4.5 text-electric-400" fill="none" stroke="currentColor" strokeWidth="1.75">
      ...
      <nav className="flex-1 space-y-1 px-3">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`h-4.5 w-4.5 ${active ? "text-electric-400" : "text-slate-500"}`} />
              {item.label}
            </Link>
          );
        })}
...
export default function ServiceCenterLayout({ children }) {
  ...
  const currentLabel = NAV_ITEMS.find((item) => isActive(item.href, pathname))?.label ?? "Dashboard";
      ...
      <div className="lg:pl-64">
        ...
        <header className="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/80 px-4 py-3.5 backdrop-blur sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-base font-semibold text-navy-900 sm:text-lg">{currentLabel}</h1>
          </div>
