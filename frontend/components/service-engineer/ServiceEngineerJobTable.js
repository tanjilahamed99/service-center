...
import { CALL_TYPE_OPTIONS, NATURE_OF_WORK_OPTIONS, STATUS_TONE, JOB_STATUS_LIST } from "../job/Constants";
...
function ActionButton({ label, iconD, onClick, tone = "slate" }) {
  const toneClass = tone === "electric" ? "text-electric-500 hover:bg-electric-500/10" : "text-slate-500 hover:bg-slate-100";
  return (
    <button
      type="button"
      onClick={onClick}
...
function AgingPill({ days }) {
  ...
  const tone = days >= 7 ? "text-red-500 bg-red-50" : days >= 3 ? "text-amber-500 bg-amber-50" : "text-slate-500 bg-slate-100";
  return (
    <span className={`inline-flex items-center whitespace-nowrap rounded-md px-2 py-1 text-xs font-medium ${tone}`}>
...
function RowActions({ job, onViewJob, onUpdateStatus }) {
  return (
    <div className="flex items-center gap-1">
      <ActionButton label="View Job" iconD="view" onClick={() => onViewJob?.(job)} />
      <ActionButton label="Update Status" iconD="status" tone="electric" onClick={() => onUpdateStatus?.(job)} />
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div className="mt-3 flex justify-end border-t border-slate-100 pt-3">
        <RowActions job={job} onViewJob={onViewJob} onUpdateStatus={onUpdateStatus} />
      </div>
    </div>
  );
}
 ...
 */
export default function ServiceEngineerJobsTable({ title, subtitle, jobs, onViewJob, onUpdateStatus }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.sort((a, b) =>
      sortDesc ? b.complaintDate?.localeCompare(a.complaintDate) : a.complaintDate?.localeCompare(b.complaintDate),
    );
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:flex-row sm:flex-wrap sm:items-center">
        ...
        <div className="grid grid-cols-2 gap-3 sm:contents">
          <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-navy-900 focus:border-electric-400 focus:bg-white focus:outline-none focus:ring-1 focus:ring-electric-400 sm:w-auto">
            <option value="">All Statuses</option>
            {JOB_STATUS_LIST.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
          </select>
          <select value={callType} onChange={(e) => setCallType(e.target.value)} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-navy-900 focus:border-electric-400 focus:bg-white focus:outline-none focus:ring-1 focus:ring-electric-400 sm:w-auto">
            <option value="">All Call Types</option>
            {CALL_TYPE_OPTIONS.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
          </select>
          <select value={nature} onChange={(e) => setNature(e.target.value)} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-navy-900 focus:border-electric-400 focus:bg-white focus:outline-none focus:ring-1 focus:ring-electric-400 sm:w-auto">
            <option value="">All Nature of Work</option>
            {NATURE_OF_WORK_OPTIONS.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
          </select>
        </div>
        ...
        <button
          ...
          <Icon d="M7 16V4M7 4L3 8M7 4l4 4M17 8v12M17 20l4-4M17 20l-4-4" className="h-4 w-4" />
      ...
      <div className="space-y-3 lg:hidden">
        {filtered.map((job) => (
          <JobCard key={job._id} job={job} onViewJob={onViewJob} onUpdateStatus={onUpdateStatus} />
        ))}
        {filtered.length === 0 && (
          ...
          <tbody>
            {filtered.map((job, idx) => (
              <tr key={job._id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60">
                ...
                <td className="whitespace-nowrap px-4 py-3 font-medium text-navy-900">{job.complaintNumber}</td>
                <td className="whitespace-nowrap px-4 py-3 text-slate-600">{job.scheduleDate}</td>
                <td className="px-4 py-3">
                  <p className="max-w-[180px] truncate font-medium text-navy-900" title={job.customer?.name}>{job.customer?.name}</p>
                  <p className="whitespace-nowrap text-xs text-slate-500">{job.customer?.mobileNumber}</p>
                </td>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan="100%" className="px-4 py-12 text-center text-sm text-slate-400">
                  No jobs match the current filters.
                </td>
              </tr>
            )}
